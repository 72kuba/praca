...
<div class="row">
    <div class="column">
        <form action="#" method="POST">
            ...
            <input type="text" class="form-control" name="input_value" name="input_value" required pattern="[a-zA-Z-]+" title="use letters only"><br>
            <label>specify name for dumpfile</label><br>
            <input type="text" class="form-control" name="input_value" name="filename" required pattern="[a-zA-Z0-9-]+" title="letters and numbers only"><br>
            <label>specify directory - use list for help:</label><br>
            <input type="text" class="form-control" name="input_value" name="directory" required pattern="[^\s]+"><br>
            <label>specify tablespace name - use list for help:</label><br>
            <input type="text" class="form-control" name="input_value" name="tablespace" required pattern="^[a-zA-Z]+$" title="starts with letter"><br><br>
...
DECLARE
  ...
  h1 NUMBER;               
BEGIN
h1 := DBMS_DATAPUMP.OPEN('EXPORT','TABLESPACE',NULL,'$job_name','$version');
...
h1   NUMBER;
begin
h1 := dbms_datapump.open(operation => 'EXPORT', job_mode => 'TABLESPACE',job_name => '$job_name'); 
...
dbms_datapump.metadata_filter(handle => h1, name => 'TABLESPACE_LIST', value => '''$filter''');
